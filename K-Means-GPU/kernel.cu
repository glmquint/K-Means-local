...
__global__ void worker(ClassedPoint* first_point, int partition_length, int thread_num)
{
	double min_d;
	int best_k;
	double dist;
...
__global__ void worker_test(ClassedPoint* d_pnt, Centroid* d_centr, int dataset_size, int num_clusters) {
	...
	if (index < dataset_size) {
		printf("%f %f\n", d_pnt[index].p.coords[0], d_pnt[index].p.coords[1]);
	}
}

void performRounds(dim3 grid, dim3 block, ClassedPoint** first_points, int* partition_lengths)
{
	int round = 0;
	while (!CONVERGED)
	{
		...
		worker_test <<< grid, block>>> (d_points, d_centroids, DATASET_SIZE, NUM_CLUSTERS);
...
int main(int argc, char** argv)
{
	...
	for (int rep = 0; rep < 1; rep++)
	{
		...
		performRounds(grid, block, first_points, partition_lengths);
